...
CREATE TABLE skills (
    ...
    -- For semantic search and AI matching
    embedding VECTOR(768), -- Assuming a 768-dimension vector from your Gemini model
    ...
    -- A skill name should be unique within a category
    UNIQUE (category_id, name)
);

-- Index for fast text search on skill names. Using GIN for trigram matching.
CREATE EXTENSION IF NOT EXISTS pg_trgm;
CREATE INDEX idx_skills_name_trgm ON skills USING GIN (name gin_trgm_ops);

-- Index for fast vector similarity search (requires pgvector)
CREATE INDEX idx_skills_embedding ON skills USING IVFFLAT (embedding vector_cosine_ops);

-- This table links users to skills. Assumes it was created with the UserService schema.
-- If not, here is the definition again.
...
CREATE TABLE user_skills (
                           ...
                           experience_description TEXT, -- Corresponds to the AddUserSkillRequest field
